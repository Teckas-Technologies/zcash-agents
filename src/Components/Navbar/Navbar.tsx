...
export default function Navbar({
  ...
  return (
    <>
      ...
      <nav
        className={`header fixed bg-white md:relative h-screen text-white p-4 flex-col justify-between transition-all ${isCollapsed && !isMobileNavVisible ? "w-20" : "w-[14rem] md:w-[15rem] lg:w-[16rem]"
        ...
        <div className="flex flex-col h-full">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 font-semibold">
              <img src="images/near-intent-logo.svg" className="h-9" />
            </div>
            ...
            <ul className="mt-6 space-y-2" style={{ fontFamily: "manrope" }}>
              <li>
                <button
                  className={`flex cursor-pointer items-center w-full gap-2 px-3 py-2 rounded-lg transition-colors ${active === "Chat" && "bg-[#21201C]"}`}
                  onClick={() => handleNavigation("Chat")}
                >
                  <FaLaptopCode className={`w-8 h-8 ${active === "Chat" ? "text-white" : "text-[#21201C]"}`} />
                  {(!isCollapsed || isMobileNavVisible) && (
                    <h2 className={`${active === "Chat" ? "text-white" : "text-[#21201C]"} text-lg font-bold`}>Chat</h2>
                  )}
                </button>
              </li>
              <li>
                <button
                  className={`flex cursor-pointer items-center w-full px-3 gap-2 py-2 rounded-lg transition-colors ${active === "Browse Agents" && "bg-[#21201C]"}`}
                  onClick={() => handleNavigation("Browse Agents")}
                >
                  <FaRobot className={`w-8 h-8 ${active === "Browse Agents" ? "text-white" : "text-[#21201C]"}`} />
                  {(!isCollapsed || isMobileNavVisible) && (
                    <h2 className={`${active === "Browse Agents" ? "text-white" : "text-[#21201C]"} md:text-lg text-md md:font-bold font-semibold`}>
                      Browse Agents
                    </h2>
                  )}
                </button>
            ...
            <div className="line my-4 w-full h-2">

            </div>
            {(!isCollapsed || isMobileNavVisible) && (
              <div className="mt-4 space-y-3">
                <div onClick={isMobileNavVisible ? () => { onMobileNavToggle(); setIsSoon(true) } : () => setIsSoon(true)} className="build-agent block flex items-center justify-between text-gray-400 text-lg font-semibold px-2 py-0 rounded-lg">
                  <Link
                    href="#"
                ...
                <div onClick={isMobileNavVisible ? () => { onMobileNavToggle(); setIsSoon(true) } : () => setIsSoon(true)} className="build-agent block flex items-center justify-between gap-1 text-gray-400 text-lg font-semibold px-2 py-0 rounded-lg">
                  <Link
                    href="#"
      ...
      {isSoon && <div onClick={() => setIsSoon(false)} className={`absolute top-0 bottom-0 right-0 left-0 bg-transparent backdrop-blur-[25px] z-10 flex justify-center items-center ${isCollapsed && !isMobileNavVisible ? "" : " md:pl-[15rem] lg:pl-[16rem]"}`}>
        <div className="center-box w-[22rem] md:w-[24rem] lg:w-[25rem] xl:w-[26rem] min-h-[12rem] md:min-h-[15rem] bg-white rounded-md agents-box shadow-lg">
          ...
          <div onClick={(e) => e.stopPropagation()} className="inside-box w-full pb-5 md:pb-[1.5rem] md:min-h-[15rem] min-[13rem] flex flex-col items-center justify-center gap-1">
            ...
            <div className="soon max-w-[10rem] mt-2 text-center px-3 py-1 bg-[#F76B15] rounded text-white md:text-sm text-[8px] font-semibold"
              style={{ fontFamily: "orbitron" }}>
              COMING SOON
            </div>
          </div>
